...
public class PreDecisionTree implements Serializable{
    ...
    // describe the subspace, that is, which features we can use in this tree.
    transient ArrayList<Integer> availableFeatureIdx = new ArrayList<Integer>();
    ...
    transient int parentFeatureIdx = -1;

    // filtered data that belongs to this node
    transient ArrayList<DataInstance> data = null;
    ...
    transient double purity = 0;


    PreDecisionTree(int l, int fidx, int pfidx, ArrayList<Integer> availableFeatureIdx) {
        level = l;
        featureIdx = fidx;
        parentFeatureIdx = pfidx;
        this.availableFeatureIdx = availableFeatureIdx;
        left = right = null;
    }
    ...
    public void split(Queue<PreDecisionTree> q) {
        ...
        if (data.size() == 0 || level == availableFeatureIdx.size() - 1 || level == -1) {
            this.level = -1;
            left = right = null;
            return;
        }
        ...
        left = new PreDecisionTree(level + 1, -1, featureIdx, availableFeatureIdx);
        right = new PreDecisionTree(level + 1, -1, featureIdx, availableFeatureIdx);
    ...
    private int evaluateIG(){
        int resultIdx = availableFeatureIdx.get(0);
        double minIG = getIG(availableFeatureIdx.get(0));
        for(int testIdx = 1; testIdx < availableFeatureIdx.size(); testIdx++) {
            double tmpIG = getIG(availableFeatureIdx.get(testIdx));
            if(minIG > tmpIG) {
                minIG = tmpIG;
                resultIdx = availableFeatureIdx.get(testIdx);
            }
        }
    ...
    public String toString (){
        ...
        if(this.left == null)
            ...
            leftInfo += this.left.level;


        if(this.right == null)
            rightInfo = "null";
        else
            rightInfo += this.right.level;

        return "Tree: accuracy=" + accuracy + "; left.level=" + leftInfo + "; right.level="+ rightInfo;
    }
}
